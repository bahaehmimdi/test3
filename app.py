    ...
    for prefix in prefixes:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                ttbl=tbl(soup)
                fj={'status': 'success','h1':soup.find('h1').get_text().strip(),'titles':{"Paragraphe":list(ttbl.keys()),"Title":list(ttbl.values())},'topic':extract_title(soup),'metas':extract_meta_tags(soup),'final':str(response.url), 'data': soup.get_text()}#,'tst':str(tst),'testedurl':testedurl,'lasturl':str(list(map(lambda a:a.url,response.history))),
